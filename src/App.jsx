    ...
    <div className="min-h-screen bg-[#0f172a] text-slate-100 flex flex-col lg:flex-row overflow-hidden font-sans">
      ...
      <aside className={`fixed lg:sticky top-0 h-screen w-80 bg-[#1e293b] border-r border-slate-700 transform transition-transform duration-300 z-40 flex flex-col shrink-0 ${isSidebarOpen ? 'translate-x-0' : '-translate-x-full lg:translate-x-0'}`}>
        <div className="p-6 flex-1 flex flex-col min-w-0 overflow-hidden">
          ...
          <div className="hidden lg:flex flex-col items-center justify-center mb-8 w-full" onClick={() => setSeciliBolum(null)}>
            <div className={`bg-[#f7df1e] w-12 h-12 rounded-xl flex items-center justify-center shadow-lg mb-4 transition-all duration-500 ${seciliBolum ? 'scale-100 opacity-100' : 'scale-0 opacity-0 h-0 mb-0'}`}>
          ...
          <nav className="flex-1 space-y-1 overflow-y-auto scrollbar-hide">
            {filteredBolumler.map((bolum) => (
              <button 
                key={bolum.id} 
                onClick={() => { setSeciliBolum(bolum); setIsSidebarOpen(false); window.scrollTo(0, 0); }} 
      ...
      {/* Main Content */}
      <main className="flex-1 overflow-y-auto h-screen bg-[#0f172a] scrollbar-hide">
        {seciliBolum ? (
          ...
          <div className="flex flex-col items-center justify-center min-h-screen text-center px-4 animate-in fade-in duration-700">
            <div className="bg-[#f7df1e] w-24 h-24 lg:w-32 lg:h-32 rounded-3xl flex items-center justify-center shadow-2xl mb-10 rotate-3">
